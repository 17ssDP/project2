...
$queryAll = "select * from artworks where orderID is NULL and ((artist like '%$searchImage%') or (title like '%$searchImage%') or (description like '%$searchImage%'))";
$queryPage = "SELECT * FROM artworks WHERE orderID is NULL and ((artist like '%$searchImage%') or (title like '%$searchImage%') or (description like '%$searchImage%')) ORDER BY ".$sortWay." limit ".$mark.", ".$pageSize."";
...
while ($row = $result->fetch_assoc()) {
    $html .= '<div class="img-infor">
                    ...
                    <div class="introduce">
                        <p><span style="color: firebrick">By:</span> '.$row['artist'].'</p>
                    </div>
